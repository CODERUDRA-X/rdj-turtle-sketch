...
class RDJSketch:
    """
    A class to draw a sketch of Robert Downey Jr. using Python's Turtle graphics.
    The sketch is created using a set of pre-defined coordinates for different facial features.
    """
    def __init__(self, speed=0):
        # Coordinates for different parts of the sketch
        ...
        self.screen.title("Robert Downey Jr. Sketch by Turtle")
        self.pen = tu.Turtle()
        self.pen.hideturtle()
    ...
    def _draw_part(self, coord, mode=1, co=(0, 0, 0), thickness=1):
        """A generic function to draw a part of the sketch."""
        rgb_color = (co[0] / 255, co[1] / 255, co[2] / 255)
        self.pen.color(rgb_color)
        self.pen.width(thickness)

        # Go to the starting point
        start_x, start_y = coord[0]
        self._go_to(start_x, start_y)

        # Fill mode for solid shapes
        if not mode:
            self.pen.fillcolor(rgb_color)
            self.pen.begin_fill()

        # Flag for handling breaks in drawing (marked by -1, -1)
        is_break = False
        for (x, y) in coord[1:]:
            if is_break:
                self._go_to(x, y)
                is_break = False
                if not mode: self.pen.begin_fill()
                continue
            
            if x == -1 and y == -1:
                is_break = True
                if not mode: self.pen.end_fill()
                continue
    ...
    def draw(self, retain=True):
        """Draws all the parts of the sketch in order."""
        self._draw_part(self.dress, mode=0)
        self._draw_part(self.face, mode=0)
        self._draw_part(self.iface, co=(255, 255, 255), mode=0)
        self._draw_part(self.mouth, mode=0)
        self._draw_part(self.nose, mode=0)
        self._draw_part(self.ebrow, mode=0)
        self._draw_part(self.lines, mode=1, thickness=2)
        self._draw_part(self.eyes, mode=0)
        self._draw_part(self.eball, mode=0, co=(255, 255, 255))
...
if __name__ == "__main__":
    sketcher = RDJSketch(speed=0) # speed=0 is fastest
    sketcher.draw()